...
class Serverless {
  ...
  // Historically the .serverless folder was used as a temporary directory, but
  // that prevents parallel packaging or deploying commands from being run at
  // the same time. A unique temporary directory is needed for each run.
  // In order to maintain backwards compatibility for v2.x.x the temporary
  // directory needs to be separate from the package output directory.
  getTmpDir() {
    const options =
      this.processedInput && this.processedInput.options ? this.processedInput.options : {};
    let serverlessTmpDirPath;
    // TODO Option name TBD
    if (options.usePackageForTmp) {
      serverlessTmpDirPath = this.getPackageOutputDir();
    } else {
      this.cli.log('falling back to old ways');
      serverlessTmpDirPath = path.join(this.config.servicePath || '.', '.serverless');
    }

    return serverlessTmpDirPath;
  }

  // All uses of `.serverless` should either point to the package
  // output directory (getPackageOutputDir), or to the temp director (getTmpDir)
  getPackageOutputDir() {
    const packageOption =
