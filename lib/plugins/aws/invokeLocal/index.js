...
class AwsInvokeLocal {
  ...
  buildDockerImage(layerPaths) {
    ...
    return fileExists(dockerfileCachePath)
      ...
      .then(cacheEquals => {
        ...
        return BbPromise.all([
          ensureDir(path.join(this.serverless.getTmpDir(), 'invokeLocal', runtime)),
  ...
  extractArtifact() {
    ...
    const destination = path.join(this.serverless.getTmpDir(), 'invokeLocal', 'artifact');
  ...
  ensurePackage() {
    if (this.options['skip-package']) {
      return fse
        .access(path.join(this.serverless.getTmpDir(), 'serverless-state.json'))
